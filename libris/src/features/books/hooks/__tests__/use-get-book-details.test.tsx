...
describe('useGetBookDetail', () => {
  ...
  it('should fetch book details successfully', async () => {
    ...
    const { result } = renderHook(() => useGetBookDetail({ bookId: 'test-id' }), { wrapper });
  ...
  it('should handle adding book to shelf', async () => {
    ...
    expect(toast.success).toHaveBeenCalledWith('detail.added');
  });
  ...
  it('should handle removing book from shelf', async () => {
    ...
    expect(toast.success).toHaveBeenCalledWith('detail.removed');
  });
  ...
  it('should return isError true when fetch fails', async () => {
    (getBookDetailsById as jest.Mock).mockRejectedValueOnce(new Error('Fetch error'));
  ...
  it('should not fetch if bookId is empty', async () => {
    const { result } = renderHook(() => useGetBookDetail({ bookId: '' }), { wrapper });
